...
export function useGroupMessages(groupId: string, currentUserId: string) {
  ...
  // دالة دمج الرسائل بدون تكرار
  const mergeMessages = (local: Message[], fromServer: Message[]) => {
    const confirmedTempIds = new Set(fromServer.map((msg) => msg.tempId).filter(Boolean));
    const filteredLocal = local.filter((msg) => !confirmedTempIds.has(msg.tempId));
    return [...filteredLocal, ...fromServer].sort(
      (a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
    );
  };
            ...
            if (data.newMessage) {
              setMessages((prev) => {
                const exists = prev.some(
                  (msg) =>
                    msg._id === data.newMessage!._id ||
                    msg.tempId === data.newMessage!.tempId
                );
                if (exists) return prev;

                return [...prev, data.newMessage!].sort(
                  (a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
                );
              });
            }
